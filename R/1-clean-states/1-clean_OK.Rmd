...
df_out$Name <- str_replace(df_out$Name, "Correctiona ", "Correctional ")
...
```{r}
##Getting rid of facilities named "-" and the vague "North Fork Correctional Center- Unit since I can't tell whether its the I unit or J unit or some other unit
df_mid <- df_mid %>% filter(Name != "-") %>% filter(Name != "NORTH FORK CORRECTIONAL CENTER- UNIT") %>%
  filter(!is.na(Name))
```
