...
```{r}
##removing house of correction, jail, and youth detention
df_mid <- df_mid %>% filter(Name != "HOUSE OF CORRECTION") %>% 
  filter(Name != "JAIL") %>% filter(Name != "YOUTH DETENTION")
df_mid <- df_mid %>%
  filter(!is.na(Name))
```
...
**MOOSE LAKE, LINO LAKES, WILLOW RIVER, seem to have major discrepancies in cases**
...
**AP is slightly different on specific days, just ask Hope about this during the meeting**
