...
test <- "https://services5.arcgis.com/6zMLz8j1ZaiDC1Ud/arcgis/rest/services/COVID_BOP_Testing_Fac_Public/FeatureServer/0/query?f=json&cacheHint=true&resultOffset=0&resultRecordCount=50&where=1%3D1&orderByFields=&outFields=*&returnGeometry=false&spatialRel=esriSpatialRelIntersects"
...
staff_cases_coalesced <- staff_cases_raw %>% 
  group_by_coalesce(., BOP_Naming_Convention, Date_transmuted, method = "sum")
...
df_mid <- out_data %>%
  ...
  group_by_coalesce(., Name, Date, method = "sum") %>% ## another group_by_coalesce for "ALL BOP RESIDENTIAL REENTRY CENTERS"
