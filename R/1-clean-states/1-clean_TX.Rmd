...
df_out %<>% 
  mutate(Staff.Deaths = behindbarstools::coalesce_with_warnings(Staff.Deaths, Staff.Death),
         Resident.Deaths = behindbarstools::coalesce_with_warnings(Resident.Death, Resident.Deaths)) 
```
...
# unknown facility names can be split into three categories:
# 1 : from website https://www.tcjs.state.tx.us
#     not much COVID data

# 2 : from website https://www.tdcj.texas.gov/covid-19/mac_dashboard.html
#   : majority of the COVID data

# 3 : from other websites, can disregard

# added facilities from category #2 to the crosswalk, disregarded the rest

# facility names that were excluded (from category #3)
# LOCAL HOSPITAL
...
# TOTAL
# UNKNOWN
# SAGUARO CORRECTIONAL CENTER (in Arizona)

df_mid <- df_mid %>% filter(name_match == TRUE)
...
df_hist_final$Residents.Deaths <- as.numeric(df_hist_final$Residents.Deaths)
...
ggplot(data = dates, 
  ...
  labs(x = "Date",
    y = "n instances")
```
Fix little data issues:

```{r}
df_hist_final_edit <- df_hist_final %>%
  filter(Date != as.Date("2020-07-07")) ## something went wrong on this day


## NB: after 9/20/20, Residents.Confirmed started dropping
```
