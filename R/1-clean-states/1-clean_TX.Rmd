---
title: "Clean Texas - COVID-19 Behind Bars Historical Data Cleaning"
author: "Hope Johnson"
date: "4/16/21"
...
# fix facilities with different values for Resident.Deaths and Resident.Death
df_out$Resident.Deaths[10737] <- 1
df_out$Resident.Deaths[10735] <- 1
df_out$Resident.Deaths[10879] <- 1
df_out$Resident.Deaths[10881] <- 1
df_out$Resident.Deaths[11023] <- 1
df_out$Resident.Deaths[11025] <- 1
```
...
df_hist_final <- df_hist %>%
  mutate(source = Website,
         Residents.Deaths = string_to_clean_numeric(Resident.Deaths)) %>%
...
```
Check out historical TX deaths in state facililities.

```{r}
my.max <- function(x) ifelse( !all(is.na(x)), max(x, na.rm=T), NA)

tx_deaths_sum <- df_hist_final %>%
  filter(Jurisdiction == "state") %>% # filters out "NA" jurisdiction I believe 
  group_by(Name, Facility.ID, month_yr = glue('{month(Date)}_{year(Date)}')) %>% 
  summarise(Residents.Deaths_max_fac_month = my.max(Residents.Deaths)) %>% # get max cumulative count from each facility in each month 
  ungroup() %>% 
  group_by(month_yr) %>% 
  summarize(Residents.Deaths_cbb_historical = sum_na_rm(Residents.Deaths_max_fac_month)) # sum max cumulatives by month

# write_csv(tx_deaths_sum, "~/Desktop/historical_tx_deaths.csv")
