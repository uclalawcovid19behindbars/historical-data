---
title: "Clean New Mexico - COVID-19 Behind Bars Historical Data Cleaning"
...
df_mid <- df_mid %>%
  filter(!is.na(Name)) %>%
  filter(Name != "DISMAS LAS CRUCES") ## federal
```
...
The interactive plots for Resident Cases and Resident Deaths look very weird. I also noticed that for some facilities, the confirmed cases plot doesn't match up with the y axis. 

When I looked through the comparison with AP/Marshall project, I noticed that our Residents.Confirmed and Residents.Recovered numbers jump from around 500 to 1000 then back to 500 again, while mpap data does not. (These jumps are reflected in the interactive plots)
---> STATEWIDE driving this, leaving it as-is

For the individual facility plots, Northeast and Western New Mexico Correctional Facilities had a -1 decrease in overall case counts, but these were quickly followed by a +1, so hopefully these numbers are not too wonky. 
