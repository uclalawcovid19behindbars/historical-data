...
```
```{r Plot Notes}
## DECISION --> HOLD OFF ON INTEGRATING THIS UNTIL WE DO A SYSTEMATIC RUN-THROUGH OF THE PRE_NOV SOURCES SAVED
...
Make data updates based on irregularities from scraping: 

```{r}
df_hist_final_edit <- df_hist_final %>%
  filter(Name != "STATEWIDE") %>% ## statewide numbers are very sus 
  filter(Date != as.Date("2020-08-31"), ## days when we only had "non-specific facilities" collected
