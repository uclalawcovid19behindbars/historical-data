...
```{r}
##Getting rid of facilities named "NON_PRISON STAFF", "NON_FACILITY", and vague correctional facilities that could not be directly identified
df_mid <- df_mid %>% 
  filter(Name != "NON-PRISON STAFF") %>% 
  filter(Name != "NON-FACILITY") %>% 
  filter(Name != "CO") %>% 
  filter(Name != "FTC") %>% 
  filter(Name != "PP OFFICES") %>% 
  filter(!is.na(Name))
```
...
First, there seem to be a significant number of facilities that have lag count cases that dip severely towards the end of sampling (maybe 4-5 facilities have cumulative cases that dip around 11/20). I tried looking through the wayback machine, but unfortunately the web pages would not load for me.

Second, the AP comparison showed some irregularities. There was a specific data point near the beginning that showed a ~5000 increase in resident cases, which immediately dips within the next couple of days. This is almost definitely a scraping error, but I just wanted to let you know. Date is 2020-04-06.

Staff.Confirmed fell a lot on 11/12/20 and in the following (few) scrapes we collected. Deleting these instances

```{r}
df_hist_final_edit <- df_hist_final %>%
  ## big dips in almost every facility
  filter(Date != as.Date("2020-11-04")) %>%
  mutate(Staff.Confirmed = ifelse(Date > "2020-11-05", NA, Staff.Confirmed)) %>%
  ## picked up residents.active as statewide residents.confirmed 
  mutate(Residents.Confirmed = ifelse(Name == "STATEWIDE", NA, Residents.Confirmed),
         ## anomaly checking review
         Residents.Confirmed = ifelse(Date == "2020-07-13" & Name == "FARMINGTON CORRECTIONAL CENTER", 
                                      NA, 
                                      Residents.Confirmed),
         Staff.Confirmed = ifelse(Name == "STATEWIDE" & Date == as.Date("2020-08-17"),
                                  NA, Staff.Confirmed),
          Staff.Confirmed = ifelse(Name == "STATEWIDE" & Date > as.Date("2020-08-23"),
                          NA, Staff.Confirmed),
         Residents.Confirmed = ifelse(Date == "2020-10-23" & Name == "JCCC JEFFERSON CITY", 
                                NA, 
                                Residents.Confirmed)
         ) 
```
