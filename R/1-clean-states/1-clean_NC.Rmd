...
Remove observations where the name was NA.
...
df_hist_final <- behindbarstools::reorder_cols(df_hist_final, rm_extra_cols = TRUE) %>%
  select_if(~sum(!is.na(.)) > 0) # rm 100% missing cols. doing this because we don't want all logical cols for the row_
...
write_csv(out, file.path(base_path, "data", "NC-historical-data.csv"))
```
