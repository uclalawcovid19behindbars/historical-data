...
Do a little facility name cleaning before the Name Cleaning

```{r}
df_scrubbed <- df_out %>%
  mutate(Name = str_replace_all(Name, "(?i)[ÄÃ]", ""))
```


```{r standardize facility names}
df_mid <- behindbarstools::clean_facility_name(df_scrubbed, debug = TRUE) 
```
...
Remove rows when we didn't get a name at all. Filter out "total". 
```{r}
df_mid <- df_mid %>%
  filter(!is.na(Name),
         !str_detect(Name, "(?i)total"),
         name_match != FALSE) # confirmed that all of these are federal
...
Remove data that was clearly mis-scraped. 

```{r}
df_hist_final <- df_hist_final %>%
  filter(Facility.ID != 454) %>% # filter out cook county jail until we re-scrape
  mutate(Residents.Deaths = ifelse(Date == as.Date("2020-09-23"), NA, Residents.Deaths), ## rm deaths on days we seem to have mis-scraped
         Residents.Deaths = ifelse(Date > as.Date("2020-10-01"), NA, Residents.Deaths))
...
Bad data scraped on this date, and it exists from the other scraper source. 
```{r}
df_hist_final_edit <- df_hist_final %>%
  filter(Date != "2021-11-04")
```
