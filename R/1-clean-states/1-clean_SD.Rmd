---
title: "Clean South Dakota - COVID-19 Behind Bars Historical Data Cleaning"
author: "Kevin Zhao and Hope Johnson"
date: "5/28/2021"
output: pdf_document
---
...
df_out %>%
  ...
  names()
df_out <- df_out %>% mutate(Staff.Deaths = behindbarstools::coalesce_with_warnings(Staff.Deaths, Staff.Death),
...
```{r}
df_mid <- df_mid %>% filter(!is.na(Name))
```
...
## staff confirmed "2020-10-18" seems to be an issue for the following facilities: 
problem_facs_10_18 <- c("MIKE DURFEE STATE PRISON SPRINGFIELD",
                        "PIERRE COMMUNITY WORK CENTER",
                        "SOUTH DAKOTA WOMENS PRISON PIERRE",
                        "SOUTH DAKOTA WOMENS PRISON UNIT E",
                        "SOUTH DAKOTA STATE PENITENTIARY")

df_hist_final_edit <- df_hist_final %>%
  filter(!(Name %in% problem_facs_10_18 & Date == as.Date("2020-10-18")),
         !(Name == "SIOUX FALLS COMMUNITY WORK CENTER" & Date == as.Date("2020-11-02")),
         !(Name == "SIOUX FALLS COMMUNITY WORK CENTER" & Date == as.Date("2020-11-04"))
  ) %>% 
  mutate(Residents.Confirmed = 
           ifelse(Name == "PIERRE COMMUNITY WORK CENTER" & Date == as.Date("2020-09-20"), 105, Residents.Confirmed),
         Residents.Confirmed = 
           ifelse(Name == "PIERRE COMMUNITY WORK CENTER" & Date == as.Date("2020-09-25"), 109, Residents.Confirmed),
         Residents.Confirmed = 
           ifelse(Name == "PIERRE COMMUNITY WORK CENTER" & Date == as.Date("2020-10-18"), 113, Residents.Confirmed),
         Residents.Confirmed = 
           ifelse(Name == "PIERRE COMMUNITY WORK CENTER" & Date == as.Date("2020-09-07"), 3, Residents.Confirmed),
         Residents.Confirmed = 
           ifelse(Name == "PIERRE COMMUNITY WORK CENTER" & Date == as.Date("2020-09-08"), 3, Residents.Confirmed),
         )
```
