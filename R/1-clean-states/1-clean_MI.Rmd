---
title: "Clean Michigan - COVID-19 Behind Bars Historical Data Cleaning"
author: "Kayla Teng"
date: "5/29/21"
...
```{r Remove facilities because no match and not necessary}
df_mid <- df_mid %>%
  filter(Name != "LOCATION") %>% # No clear match to any facilities
  filter(Name != "TOTA") # Assuming this means TOTAL cases which is                           not necessary
```
...
```{r Plot Notes}
## Cannot verify the historical counts because missing raw data source
```
...
# On 2020-08-12, there were 35 instances. But, on 2020-08-14, went down to 34 instances. 2020-08-13 was not recorded in Excel sheet, so unsure if instances went down on 8/14 or 8/13. On 2020-08-17, instances went back up to 35.
# From 2020-09-20 to 2020-10-11, instances were fluctuating up and down. One notable span is from 2020-10-05 to 2020-10-11. On 2020-10-05, there were 37 instances. On 2020-10-07, there were 30 instances. On 2020-10-11, there were 39 instances. 2020-10-06, 2020-10-08, 2020-10-09, and 2020-10-10 were missing, so do not know where exactly instances started rising and falling.
# On 2020-10-26, there were 41 instances. But, on 2020-10-28, went down to 39 instances.
```
...
```{r Comparison Notes}
# cbb does not have Staff.Recovered recorded in Excel sheet
# Residents Confirmed (2020-04-08): 262 (cbb) vs. 305 (mpap)
# Residents Confirmed (2020-09-01): 5230 (cbb) vs. 5160 (mpap)
# Residents Deaths (2020-10-20): 635 (cbb) vs. 74 (mpap)
# CBB recorded 713 Resident Deaths on 2020-10-23, but recorded 74 Resident Deaths on 2020-10-26.
# Residents Recovered (2020-06-23): 2236 (cbb) vs. 2367 (mpap)
# Residents Recovered (2020-06-30): 2367 (cbb) vs. 2667 (mpap)
# Residents Recovered (2020-07-07): 2667 (cbb) vs. 3011 (mpap)
# Residents Recovered (2020-07-14): 2667 (cbb) vs. 3076 (mpap)
# Residents Recovered (2020-07-21): 2667 (cbb) vs. 3585 (mpap)
# Residents Recovered (2020-08-25): 3600 (cbb) vs. 3786 (mpap)
# Residents Recovered (2020-09-15): 3600 (cbb) vs. 4027 (mpap)
# Residents Recovered (2020-09-29): 3600 (cbb) vs. 4693 (mpap)
```

```{r}
## huge outliers in the following facilities/dates:
## IONIA CORRECTIONAL FACILTY, "2020-10-23"
## MARQUETTE BRANCH PRISON, "2020-10-20", "2020-10-23"
## RICHARD A HANDLON CORRECTIONAL FACILITY, "2020-10-20", "2020-10-23"

df_hist_final_edit <- df_hist_final %>%
  mutate(Residents.Deaths = ifelse(Date == as.Date("2020-10-20") & Name %in% c("MARQUETTE BRANCH PRISON",
                                                                               "RICHARD A HANDLON CORRECTIONAL FACILITY"),
                                   NA,
                                   Residents.Deaths),
         Residents.Deaths = ifelse(Date == as.Date("2020-10-23") & Name %in% c("MARQUETTE BRANCH PRISON",
                                                                               "RICHARD A HANDLON CORRECTIONAL FACILITY",
                                                                               "IONIA CORRECTIONAL FACILTY"),
                                   NA,
                                   Residents.Deaths),
         ) 
