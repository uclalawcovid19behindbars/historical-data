---
title: "Clean West Virginia - COVID-19 Behind Bars Historical Data Cleaning"
...
# remove error from scraping
df_mid <- df_mid %>% filter(Name != "CORRECTIONA CENTERS")
```
...
Remove funky data from one facility, date:

```{r}
df_hist_final_edit <- df_hist_final %>%
  mutate(Residents.Confirmed = ifelse(Date == as.Date("2020-09-01") & 
                                      Name == "MOUNT OLIVE CORRECTIONAL CENTER",
               NA,
               Residents.Confirmed),
         Staff.Confirmed = ifelse(Name == "STATEWIDE" & Date > as.Date("2020-08-21"),
                                  NA,
                                  Staff.Confirmed)
        ) %>%
...
ourdat_tocompare <- df_hist_final_edit %>% 
  ...
  summarise(Residents.Confirmed = sum_na_rm(Residents.Confirmed),
            ...
            # Staff.Confirmed = sum_na-rm(Staff.Confirmed),
