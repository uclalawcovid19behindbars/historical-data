---
title: "Clean Washington - COVID-19 Behind Bars Historical Data Cleaning"
author: "Hope Johnson"
date: "3/22/21"
...
Spot-fix one facility where we scraped residents.population but called it residents.confirmed. 

```{r}
df_hist_final <- df_hist_final %>%
  mutate(Residents.Confirmed = ifelse(Name == "AIRWAY HEIGHTS CORRECTIONS CENTER",
                                      NA,
                                      Residents.Confirmed))
...
Spot corrections for staff data misscrapes: 

```{r}
df_hist_final_edit <- df_hist_final %>%
  mutate(Staff.Confirmed = ifelse(Date > as.Date("2020-08-16") & Date < as.Date("2020-08-30"),
                                  NA, Staff.Confirmed),
         Staff.Confirmed = ifelse(Date == as.Date("2020-09-01"),
                                  NA, Staff.Confirmed),
         Staff.Deaths = ifelse(Date > as.Date("2020-08-16") & Date < as.Date("2020-08-30"),
                                  NA, Staff.Deaths),
         Staff.Deaths = ifelse(Date == as.Date("2020-09-01"),
                                  NA, Staff.Deaths),
         Residents.Active = NA,
         Residents.Active = ifelse(Name == "SPOKANE COUNTY JAIL", Residents.Confirmed, Residents.Active),
         Residents.Confirmed = ifelse(Name == "SPOKANE COUNTY JAIL", NA, Residents.Confirmed))
```
