---
title: "Clean MA - COVID-19 Behind Bars Historical Data Cleaning"
author: "Meichen Chen"
date: "4/08/21"
...
```
Deal with "statewide" data between 4/11/20 and 4/15/20. For these five dates collected, the number of confirmed cases and deaths among incarcerated people were all mistakenly reported as zero. 
```{r}
df_hist_final <- df_hist_final %>%
  mutate(Residents.Deaths = ifelse(Date > as.Date("2020-04-07") & Name == "STATEWIDE",
                                   NA,
                                   Residents.Deaths),
         Residents.Confirmed = ifelse(Date > as.Date("2020-04-07") & Name == "STATEWIDE",
                                   NA,
                                   Residents.Confirmed))
...
`Residents.Tested` should be `Residents.Tadmin` in the historical data file.

```{r}
df_hist_edited <- df_hist_final %>%
