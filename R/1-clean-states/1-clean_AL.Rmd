...
negative_cases <- testing_file %>% 
  filter(Date == as.Date("2020-08-26") & lag_change_cases < 0) %>%
  pull(Name)

final_dat %>%
  filter(Date == as.Date("2020-08-26") & (Name %in% negative_cases)) %>%
  select(Name, Date, Residents.Confirmed) %>%
  head()

# make suspect scraping day NA for a few facilities
final_dat <- final_dat %>%
  mutate(Residents.Confirmed = ifelse(((Name %in% negative_cases) & (Date == as.Date("2020-08-26"))), 
                                      NA,
                                      Residents.Confirmed))

```

Remove obvious data entry issue. 

```{r}
case_check <- testing_file %>% 
  filter(Date == as.Date("2020-08-26") | Date == as.Date("2020-08-27")) %>%
  group_by(Name) %>% 
  mutate(two_day_deaths = behindbarstools::sum_na_rm(lag_change_cases)) %>% 
  filter((two_day_deaths == 0) & (first(abs(lag_change_cases)) > 1)) %>%
  ungroup()
...
# remove cases from date that was obviously a data entry error
final_dat <- final_dat %>%
  filter(!((Name %in% bad_facs) & between(Date, as.Date("2020-08-26"), as.Date("2020-08-27"))))
```

```{r}
# lag deaths overall
resident_deaths <- testing_file %>%
  filter(Name != "STATEWIDE") %>%
  ggplot(data = ., 
    ...
    labs(x = "Date",
      y = "lag_change_deaths")
ggplotly(resident_deaths)

testing_file <- flag_noncumulative_deaths(testing_file, Name, Staff.Deaths)
staff_deaths <- testing_file %>%
  filter(Name != "STATEWIDE") %>%
  ggplot(data = ., 
         aes(x = Date, y = lag_change_deaths, group = Name)) +
    geom_line(alpha=0.6 , size=.5, color = "black") +
