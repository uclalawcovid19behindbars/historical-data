...
## checked rows with differing values and selected the former
```
...
df_hist_final_edit <- df_hist_final %>%
  mutate(Residents.Deaths = ifelse(Name == "JAMES T VAUGHN CORRECTIONAL CENTER" & 
                                     Date > as.Date("2020-06-25") & 
                                     Date < as.Date("2020-07-01"),
                                   7, Residents.Deaths),
         Residents.Deaths = ifelse(Name == "JAMES T VAUGHN CORRECTIONAL CENTER" & 
                                     Date == as.Date("2020-06-11"),
                                   7, Residents.Deaths)
)
## can't tell where we collected this from, and when non-NA, it's also zero (diff than MP reported 1 death)
