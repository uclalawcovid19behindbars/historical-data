---
title: "Clean Oregon - COVID-19 Behind Bars Historical Data Cleaning"
author: "Hope Johnson"
date: "3/22/21"
...
Remove rows counting staff cases outside of the prison context (4 observations). Remove federal prison that snuck in. 
```{r}
df_mid <- df_mid %>%
  filter(Name != "NON INSTITUTIONAL LOCATIONS",
         Name != "SHERIDAN FEDERAL CORRECTIONAL INSTITUTION")
```
...
df_hist_final <- df_hist %>%
  ...
  select(-c(Website, Count.ID, Resident.Deaths, Facility)) 
...
Spot corrections from anomaly checking:
```{r}
df_hist_final_edit <- df_hist_final %>%
  mutate(Staff.Confirmed = ifelse(Name == "OREGON STATE PENITENTIARY" & Date == as.Date("2020-06-26"),
                                  36,
                                  Staff.Confirmed),
         Staff.Confirmed = ifelse(Name == "OREGON STATE PENITENTIARY" & Date == as.Date("2020-06-27"),
                                  36,
                                  Staff.Confirmed))
```
