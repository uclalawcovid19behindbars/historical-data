...
```{r COMMENTS BY DARA}
  # recover: for non-NA and non-zero observations, values seem to differ quite
    # substantially -> probably different variables
  # WBM: https://web.archive.org/web/20200630001933/https://coronavirus.ohio.gov/static/reports/DRCCOVID-19Information.pdf
    # Residents.Recovered matches WBM column, Resident.Recoveries unidentifiable from WBM
df_out %>%
  filter(!is.na(Residents.Recovered), !is.na(Resident.Recoveries)) %>%
  select(sheet_name, Name, Residents.Recovered, Resident.Recoveries)

  # quarantine: for non-NA and non-zero observations, values seem to differ quite
    # substantially -> probably different variables
df_out %>%
  filter(!is.na(Residents.Quarantined), !is.na(Residents.Quarantine)) %>%
  select(Residents.Quarantined, Residents.Quarantine)
```
...
```{r COMMENTS BY DARA}
  # higher value: resident death with value 23
df_out[2350, c("Resident.Deaths", "Resident.Death")]
  # manually assign 23
df_out[2350, "Resident.Deaths"] <- 23
```
...
# COMMENTS BY DARA
  # weirdness observed especially in lag plots
  # more details in facility plots
```
...
# NOTES ON FACILITY-SPECIFIC PLOTS

## CASE PLOTS

Most problematic facilities, as determined by (1) frequency and/or (2) size of dips below zero:
...
- NORTHEAST REINTEGRATION CENTER (two large dips around 07/20) *
- PICKAWAY CORRECTIONAL INSTITUTION
- SOUTHEASTERN CORRECTIONAL INSTITUTION (large dip around 07/20) *
- TRUMBULL CORRECTIONAL INSTITUTION

## DEATH PLOTS

Most facilities are not problematic (at most one/two dips below zero of size -1/-2), other than:
- NORTHEAST REINTEGRATION CENTER *
- SOUTHEASTERN CORRECTIONAL INSTITUTION *
- OHIO STATE PENITENTIARY (one dip below of size -7 on 2020-04-21), WBM not available

* Note, the date and size of the dips below zero in the death plots for these two facilities coincide with the dips in the case plots, i.e. the dips in the case plots are due to the dips in the death counts recorded.

```{r}
## DECISION: 
# - data is too messy to display. request this directly from the state if possible

# large_dips <- c("BELMONT CORRECTIONAL INSTITUTION",
#   "SOUTHEASTERN CORRECTIONAL INSTITUTION" ,
#   "LAKE ERIE CORRECTIONAL INSTITUTION",
#   "PICKAWAY CORRECTIONAL INSTITUTION",
#   "NORTH CENTRAL CORRECTIONAL CENTER",
...
- 2020-04-29: CBB = 3855; MPAP = 3890
- 2020-05-06: CBB = 4336; MPAP = 4312
- For every date starting 2020-05-20 that MPAP has data for Staff Recovered, CBB and MPAP differ by at least 20 and the difference seems to gets larger over time.

## Residents Deaths

- 2020-06-30: CBB = 148, MPAP = 82
