---
title: "Clean New York - COVID-19 Behind Bars Historical Data Cleaning"
author: "Hope Johnson"
date: "9/3/21"
...
dir.create(file.path(plot_path, "cumulative"))
...
# plot cumulative counts by facility
cumulative_case_plots <- plot_lags(testing_cases, "Date", 
                            y_var = "Residents.Confirmed",
                            grp_var = Name)
# cumulative_case_plots$plot # view them by un-commenting this

## save cumulative case plots
for (i in 1:nrow(cumulative_case_plots)){
  facility_name <- cumulative_case_plots$Name[[i]]
  ggsave(paste0(facility_name, "_CumulativeCases.png"), cumulative_case_plots$plot[[i]],
         path = file.path(base_path, "plots", state_abbrev, "cumulative"))
}
```

Fix issues with falling cumulative counts and remove facilities with wholly terrible data quality:
```{r}
filter_out <- c("AUBURN CORRECTIONAL FACILITY",
                "BARE HILL CORRECTIONAL FACILITY",
                "CLINTON CORRECTIONAL FACILITY",
                "COXSACKIE CORRECTIONAL FACILITY",
                "FIVE POINTS CORRECTIONAL FACILITY",
                "MOHAWK WALSH REGIONAL MEDICAL UNIT",
                "MOHAWKIWALSH RMU",
...
df_hist_final_corrected <- df_hist_final %>%
  ...
  mutate(Residents.Confirmed = 
           ifelse(Name == "ADIRONDACK CORRECTIONAL FACILITY" & 
                    (Date > as.Date("2020-06-14") & Date < as.Date("2020-07-09")),
                  1, Residents.Confirmed),
         Residents.Confirmed = ifelse(Name == "COLLINS CORRECTIONAL FACILITY" & 
                  (Date > as.Date("2020-05-16") & Date < as.Date("2020-06-20")),
                  1, Residents.Confirmed),
        Residents.Confirmed = ifelse(Name == "DOWNSTATE CORRECTIONAL FACILITY" & 
            (Date > as.Date("2020-06-01") & Date < as.Date("2020-07-01")),
          1, Residents.Confirmed),
        Residents.Confirmed = ifelse(Name == "MARCY CORRECTIONAL FACILITY" & 
            (Date > as.Date("2020-06-11")),
          8, Residents.Confirmed),
       Residents.Confirmed = ifelse(Name == "QUEENSBORO CORRECTIONAL FACILITY" & 
            (Date == as.Date("2020-06-19")),
          6, Residents.Confirmed),
      Residents.Confirmed = ifelse(Name == "SOUTHPORT CORRECTIONAL FACILITY" & 
            (Date == as.Date("2020-06-01")),
          11, Residents.Confirmed),
      Residents.Confirmed = ifelse(Name == "WENDE CORRECTIONAL FACILITY" & 
            (Date > as.Date("2020-06-01") & Date < as.Date("2020-07-23")),
          29, Residents.Confirmed),
      Residents.Active = NA,
      Residents.Active = ifelse(Name == "NEW YORK CITY JAILS", Residents.Confirmed, Residents.Active),
      Residents.Confirmed = ifelse(Name == "NEW YORK CITY JAILS", NA, Residents.Confirmed),
        )
```
