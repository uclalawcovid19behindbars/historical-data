---
title: "Clean Connecticut - COVID-19 Behind Bars Historical Data Cleaning"
author: "Hope Johnson"
date: "2/24/21"
...
df_mid <- df_mid %>%
  filter(!is.na(Name),
         Name != "HOSPITAL CT", # rm federal obs
         Name != "DISCHARGED",  # rm ambiguous facility 
         Name != "FACILITY")   # rm ambiguous facility 
```
...
dir.create(file.path(plot_path, "cumulative cases"))
...
cumulative_case_plots <- plot_lags(testing_cases, "Date", 
                            y_var = "Residents.Confirmed",
                            grp_var = Name)
## save lag case plots
for (i in 1:nrow(cumulative_case_plots)){
  facility_name <- cumulative_case_plots$Name[[i]]
  ggsave(paste0(facility_name, "_Cases.png"), cumulative_case_plots$plot[[i]],
         path = file.path(base_path, "plots", state_abbrev, "cumulative cases"))
}
...
Remove dates with non-cumulative counts

```{r}
facs_with_july_mistake <- c("HARTFORD CORRECTIONAL CENTER",
                            "MACDOUGALL WALKER CORRECTIONAL INSTITUTION",
                            "MANSON YOUTH CORRECTIONAL CENTER",
                            "NEW HAVEN CORRECTIONAL CENTER",
                            "NORTHERN CORRECTIONAL INSTITUTION")

df_hist_final <- df_hist_final %>%
  mutate(Residents.Confirmed = ifelse((Name %in% facs_with_july_mistake) & 
                                        Date == as.Date("2020-07-21"),
                                      NA, 
                                      Residents.Confirmed)) %>%
    mutate(Residents.Confirmed = ifelse((Name == "HARTFORD CORRECTIONAL CENTER") & 
                                        (Date == as.Date("2020-07-04") | Date == as.Date("2020-07-05")),
                                      NA, 
                                      Residents.Confirmed)) %>%
    mutate(Residents.Confirmed = ifelse((Name == "NORTHERN CORRECTIONAL INSTITUTION") & 
                                        (Date == as.Date("2020-04-13")),
                                      NA, 
                                      Residents.Confirmed)) %>%
    mutate(Residents.Confirmed = ifelse(((Name == "CARL ROBINSON CORRECTIONAL INSTITUTION") | 
                                                 (Name == "OSBORN CORRECTIONAL INSTITUTION") ) & 
                                        (Date == as.Date("2020-04-10") | 
                                           Date == as.Date("2020-04-11") | 
                                           Date == as.Date("2020-04-12") | 
                                           Date == as.Date("2020-04-13")),
                                      NA, 
                                      Residents.Confirmed)) %>%
      mutate(Residents.Confirmed = ifelse((Name == "BRIDGEPORT CORRECTIONAL CENTER") & 
                                        (Date == as.Date("2020-04-13")),
                                      NA, 
                                      Residents.Confirmed)) %>%
      mutate(Residents.Confirmed = ifelse((Name == "CHESHIRE CORRECTIONAL INSTITUTION") & 
                                        (Date == as.Date("2020-04-02")),
                                      NA, 
                                      Residents.Confirmed)) %>%
      mutate(Residents.Confirmed = ifelse((Name == "CORRIGAN RADGOWSKI CORRECTIONAL CENTER") & 
                                        (Date == as.Date("2020-04-07") | Date == as.Date("2020-04-08")),
                                      NA, 
                                      Residents.Confirmed)) %>%
      mutate(Residents.Confirmed = ifelse((Name == "WILLARD CYBULSKI CORRECTIONAL INSTITUTION") & 
                                        (Date == as.Date("2020-04-08") | 
                                           Date == as.Date("2020-04-11") | 
                                           Date == as.Date("2020-04-12")),
                                      NA, 
                                      Residents.Confirmed)) %>%
        mutate(Residents.Confirmed = ifelse((Name == "STATEWIDE") & 
                                        (Date > as.Date("2020-06-03")),
                                      NA, 
                                      Residents.Confirmed))
  
facs_with_april_mistake <- c("BRIDGEPORT CORRECTIONAL CENTER",
                             "CARL ROBINSON CORRECTIONAL INSTITUTION",
                             "CHESHIRE CORRECTIONAL INSTITUTION",
                             "CORRIGAN RADGOWSKI CORRECTIONAL CENTER",
                             "HARTFORD CORRECTIONAL CENTER", # too confusing to fix -- active?
                             "NEW HAVEN CORRECTIONAL CENTER",
                             "OSBORN CORRECTIONAL INSTITUTION",
                             "WILLARD CYBULSKI CORRECTIONAL INSTITUTION",
                             "YORK CORRECTIONAL INSTITUTION") # too small to fix 
...
Fix issues that came up in facility vetting:

```{r}
df_hist_final_edit <- df_hist_final %>%
  filter(!((Date < as.Date("2020-04-14")) & Name == "NORTHERN CORRECTIONAL INSTITUTION")) %>%
  mutate(Residents.Confirmed = ifelse(Name == "NORTHERN CORRECTIONAL INSTITUTION" & Date < "2020-06-17",
                                      0, Residents.Confirmed))
```
