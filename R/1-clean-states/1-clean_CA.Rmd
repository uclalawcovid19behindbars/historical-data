...
df_mid <- behindbarstools::clean_facility_name(df_cln) %>%
  select(-name_xwalk)
```
...
recent_dat_cln <- recent_dat_cln %>%
  mutate(Name = ifelse(scrape_name_clean == "STATEWIDE", "STATEWIDE", Name)) %>%
  filter(Name != "NULL")
...
```{r write csv}
final_dat %<>%
  select(-c(previous_date_value_deaths, lag_change_deaths, cumulative_deaths))
