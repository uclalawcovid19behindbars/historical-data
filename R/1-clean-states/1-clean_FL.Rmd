...
##Define package list
Packages<-c("tidyverse", "glue", "assertthat", "stringr")
.packages = Packages
...
# No other "GEO groups" reported in FL 
df_mid %>%
...
## ! 
## Remove "GEO GROUP" with no associated values that are tied
df_mid <- df_mid %>%
  filter(!str_detect(Name, regex('geo', ignore_case = TRUE))) 
```
...
Find time periods where facilities were missing lots of data. 
...
df_comb %>%
  count(facility_name_clean, sort = TRUE)
```



Find non-cumulative counts, separate those into "active" and "confirmed". Use residents recovered to calculate "confirmed". 
...
ggplot(data = df_comb, 
       aes(x = date, y = lag_change, group = facility_name_clean)) +
  geom_area(alpha=0.6 , size=.5, color = "white") +
  labs(x = "Date",
    y = "lag_change")

ggplot(df_comb, aes(x=date, y=lag_change)) +
...
df_comb %>% 
  ...
  ggplot(., aes(x=date, y=Residents.Confirmed)) +
    ...
    xlab("") +
    scale_x_date(date_labels = "%b") + 
    ggtitle(unique("ST LUCE COUNTY JAIL"))

df_comb %>% 
  filter(facility_name_clean == "MIAMI FDC") %>% 
  ggplot(., aes(x=date, y=Residents.Confirmed)) +
    geom_point() +
    xlab("") +
    scale_x_date(date_labels = "%b") + 
    ggtitle(unique("MIAMI FDC"))
