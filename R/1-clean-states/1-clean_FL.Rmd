...
weekly_avg <- df_comb %>%
  group_by(week = week(date), facility_name_clean) %>%
  summarise(weekly_mean_change = mean(lag_change_deaths))
...
df_comb %>% 
  filter(facility_name_clean == "SFRC") %>% 
  ggplot(., aes(x=date, y=lag_change_cases)) +
    geom_point() +
    xlab("") +
