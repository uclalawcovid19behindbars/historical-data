...
```{r plot cases/deaths}
df_comb <- flag_noncumulative_cases(df_comb, facility_name_clean)
df_comb <- flag_noncumulative_deaths(df_comb, facility_name_clean, Resident.Deaths)
...
df_sw <- flag_noncumulative_deaths(df_sw, facility_name_clean, Resident.Deaths)
lag_death_plots <- plot_lags(df_sw, "date", 
                             y_var = "lag_change_deaths",
                             grp_var = facility_name_clean)
df_sw %>%
  filter(cumulative_deaths == "FALSE") %>%
  select(date, facility_name_clean, Resident.Deaths, previous_date_value_deaths)
...
percentage_change <- df_sw %>%
    dplyr::arrange(facility_name_clean, date) %>%
    mutate(
    ...
    select(State, facility_name_clean, date, Residents.Confirmed, 
...
## COLUMBIA CI
df_sw %>%
  filter(facility_name_clean == "COLUMBIA CI") %>%
  ggplot(data = ., 
...
## MAYO ANNEX
df_sw %>%
  filter(facility_name_clean == "MAYO ANNEX") %>%
  ggplot(data = ., 
         aes(x = date, y = Residents.Confirmed)) +
      geom_line(linetype = "dashed")+
      geom_point() + 
...
## SANTA ROSA CI 
df_sw %>%
  filter(facility_name_clean == "SANTA ROSA CI") %>%
  ggplot(data = ., 
         aes(x = date, y = Residents.Confirmed)) +
      geom_line(linetype = "dashed")+
      geom_point() + 
...
Filter down the columns to only the ones we want to save.

```{r}
## Vars not present that ARE in current scraped data: 
## Staff.Tested, Staff.Negative, Staff.Pending, Staff.Quarantine
## Residents.Released (newly scraped only), Residents.Active (historical scraped only)

out <- df_sw %>%
  select(
    date, State, facility_name_clean,
    Residents.Confirmed, Staff.Confirmed, 
    Resident.Deaths, Staff.Deaths,
    Residents.Recovered, Staff.Recovered, # what to do with these? make all null?
    Residents.Negative, 
    Residents.Tested,                     # what to do about Residents.Tested vs T.Admin
    Residents.Pending, 
    Resident.Population, 
    Residents.Quarantine, 
    Website, Facility, hifld_id, Address, Zipcode, City, County, 
    Latitude, Longitude, County.FIPS, Notes
    ) %>%  
  mutate(jurisdiction = "") %>%                # mapping from this to county, federal, state
  rename(
    Date = date,
  ...
  mutate(
    State = behindbarstools::translate_state(StateLong, reverse = TRUE)
    ) %>%
  select(-c(StateLong, Facility)) 

#write_csv(out, file.path(base_path, "data", "florida_adult_facility_covid_counts_historical.csv"))
...
```{r}
all_dat <- bind_rows(hist_to_bind, clean_recent_dat)
